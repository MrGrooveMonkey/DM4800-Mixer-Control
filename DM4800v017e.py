...
class CustomFaderSlider(QtWidgets.QSlider):
    ...
    def _get_fader_style(self):
        ...
        margin_top = "-10px" if self.is_stereo else "-5px"
        ...
        return f"""
            ...
            QSlider:handle:vertical {{
                background: {color}; border: 1px solid #444444; height: 20px; width: 24px; margin: {margin_top} -8px; border-radius: 3px;
            }}
...
class FaderWithScale(QtWidgets.QWidget):
    ...
    def __init__(self, initial=FADER_ZERO_DB_CC, h_scale=1.0, v_scale=1.0, is_master=False, is_stereo=False, parent=None):
        ...
        # Adjust margins based on stereo status
        if is_stereo:
            # Give stereo faders extra top margin to shift everything down
            layout.setContentsMargins(1,8,0,2)  # Increased top margin from 2 to 8
        else:
            layout.setContentsMargins(1,2,0,2)  # Keep regular margin
        layout.setSpacing(0)
    ...
    def apply_scale(self, h_scale: float, v_scale: float):
        ...
        # Give stereo faders extra height to accommodate repositioning
        if self.is_stereo:
            fader_h += 12  # Add extra height for stereo faders
...
class ChannelStrip(QtWidgets.QFrame):
    ...
    def __init__(self, title: str, scribble_key: str = "", has_pan: bool=True, has_mute: bool=True, has_scribble: bool=True, h_scale: float=1.0, v_scale: float=1.0, is_master: bool=False, mono_stereo_manager=None, is_stereo_pair: bool=False, stereo_partner_num: int=0, parent=None):
        ...
        # Add stereo indicator badge for stereo pairs
        #if is_stereo_pair:
        ...
        if has_pan:
            ...
            self.pan_label.setStyleSheet("font-size: 12px; color: rgb(60,60,60); font-weight: bold;")
    ...
    def apply_scale(self, h_scale: float, v_scale: float):
        ...
        if self.is_master:
            fixed_width = 60
            title_f = 12  # Larger font for "Main" label
        else:
            fixed_width = 75
            title_f = 8   # Regular font for other strips

        self.setMinimumWidth(fixed_width)
        self.setMaximumWidth(fixed_width)

        small_f = 7  # Fixed smaller detail font
        ...
        if self.pan:
            size = 30  # Fixed smaller pan dial size
...
class WideStereoStrip(QtWidgets.QFrame):
    ...
    def __init__(self, title: str, left_scribble_key: str = "", right_scribble_key: str = "", 
        ...
        if has_pan:
            ...
            self.pan_label.setAlignment(QtCore.Qt.AlignCenter)
            self.pan_label.setStyleSheet("font-size: 10px; color: rgb(20,20,225); font-weight: bold;")
            self.v.addWidget(self.pan_label)
        ...
        self.fader = FaderWithScale(initial=FADER_ZERO_DB_CC, h_scale=self.h_scale_factor, 
                                  v_scale=self.v_scale_factor, is_master=False)
        ...
        if hasattr(self.fader, 'scale'):
            ...
            def custom_paint_event(event):
                ...
                for cc, label in scale_points:
                    y = y_for_cc(cc) + 3  # Add 3px offset for wide stereo strips
                    painter.drawLine(rect.right()-6, y, rect.right()-2, y)
                    text_rect = QtCore.QRect(rect.left(), y-8, rect.width()-8, 16)
    ...
    def apply_scale(self, h_scale: float, v_scale: float):
        ...
        title_f = 8   # Fixed smaller title font
        small_f = 7   # Fixed smaller detail font
...
class MixerWindow(QtWidgets.QMainWindow):
    def __init__(self, csv_path: str):
        super().__init__()
        self.setWindowTitle("DM4800 Mixer Controller - BETA CODE: NOT PRODUCTION READY YET")
        ...
        self.h_scale_factor = 1.0  # Leaving in place because it would be a lot of work to clean out the remaining h_scale elements and its working fine as is.
    ...
    def _select_midi_in(self):
        ...
        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            ...
            #print(f"DEBUG: Selected and connected to input: {selected_port}")  # DEBUG
            if selected_port:
                ...
                if success and self.settings.get('remember_ports', True):
                    ...
                    self.settings.save_settings()
                    #print(f"DEBUG: Saved input port setting: {selected_port}")  # DEBUG
    ...
    def _select_midi_out(self):
        ...
        if dialog.exec_() == QtWidgets.QDialog.Accepted:
            selected_port = dialog.get_selected_port()
            #print(f"DEBUG: Selected and connected to output: {selected_port}")  # DEBUG
            if selected_port:
                success = self.midi.open_output(selected_port)
                if success and self.settings.get('remember_ports', True):
                    self.settings.set('midi_out_port', selected_port)
                    self.settings.save_settings()
                    #print(f"DEBUG: Saved output port setting: {selected_port}")  # DEBUG
    ...
    def _toggle_stereo_display_mode(self):
        ...
        # For now, just print the mode change - we'll implement switching later
        # Rebuild interface with new mode        
        self._rebuild_interface()
        #print(f"Stereo display mode changed to: {new_mode}")
    ...
    def _build_rows(self):
        ...
        #print(f"DEBUG: use_wide_stereo = {use_wide_stereo}, stereo_display_mode = {self.settings.get('stereo_display_mode')}")  # DEBUG
            ...
            while ch <= end and strips_added < 24:
                ...
                if self.mono_stereo_manager.is_stereo_pair(scribble_key):
                    if self.mono_stereo_manager.is_stereo_left(scribble_key):
                        ...
                        if use_wide_stereo:
                            #print(f"DEBUG: Creating wide stereo strip for Ch {ch}")  # DEBUG
                            ...
                            #print(f"DEBUG: Creating linked pair strips for Ch {ch} and {partner_num}")  # DEBUG
                            #print(f"DEBUG: partner_key = {partner_key}")  # DEBUG
                            ...
                            #print(f"DEBUG: Created right_strip for Ch {partner_num}, title: {right_strip.title_label.text()}")  # DEBUG                         
                            ...
                            #print(f"DEBUG: Added right_strip Ch {partner_num} to layout and widgets")  # DEBUG
                            
                            # Link the stereo partners
                            left_strip.set_stereo_partner(right_strip)
                            right_strip.set_stereo_partner(left_strip)
                            
                        strips_added += 2
                        ch += 2
                        #print(f"DEBUG: channel_widgets keys after adding pair: {sorted([k for k in self.channel_widgets.keys() if k[0] == 'channel'])}")  # DEBUG
                    else:
                        ch += 1
                else:
    ...
    def _wire_strip_controls(self, section: str, number: int, strip: ChannelStrip):
            ...
            #print(f"DEBUG: Wiring {key} - is_stereo:{is_stereo}, has_partner:{has_partner}, mode:{mode_check}, is_wide:{is_wide_stereo}")
            ...
            if strip.pan:
                try:
                    ...
                    pass
            #print(f"DEBUG: Wiring {key}, has_is_stereo_pair: {hasattr(strip, 'is_stereo_pair')}, is_stereo_pair: {getattr(strip, 'is_stereo_pair', 'N/A')}")
            ...
            if (hasattr(strip, 'is_stereo_pair') and strip.is_stereo_pair and hasattr(strip, 'stereo_partner_strip') and strip.stereo_partner_strip and
                ...
                if left_key:
                    ...
                    if strip.pan:
                        ...
                        if left_pan_mapping:
                            strip.pan.valueChanged.connect(lambda v, m=left_pan_mapping: self.midi.send_cc(m.midi_channel, m.cc_number, v))
            elif not (hasattr(strip, 'is_stereo_pair') and strip.is_stereo_pair and hasattr(strip, 'stereo_partner_strip') and strip.stereo_partner_strip and
                     self.settings.get('stereo_display_mode', 'linked_pair') == 'linked_pair'):
                #print(f"DEBUG: Wiring {key} as regular mono strip")
                # Regular non-stereo strips use their own MIDI mappings
                m = map_for('fader')
                if m:
            ...
            # Add stereo synchronization for linked pairs
            if hasattr(strip, 'is_stereo_pair') and strip.is_stereo_pair and strip.stereo_partner_strip:
                # Connect fader sync
                strip.fader.valueChanged.connect(lambda v: strip._sync_fader_to_partner(v))
            
                # Connect pan sync
                if strip.pan:
                    strip.pan.valueChanged.connect(lambda v: strip._sync_pan_to_partner(v))
            
                # Connect mute sync
                if strip.mute:
                    strip.mute.toggled.connect(lambda v: strip._sync_mute_to_partner(v))
                          
    def _setup_stereo_pairs(self):
            # Setup channel stereo pairs
            for (section, number), strip in self.channel_widgets.items():
                if section == "channel":
                    ...
                    if partner_key:
                        ...
                        partner_strip = self.channel_widgets.get(("channel", partner_num))
                        if partner_strip:
                            strip.set_stereo_partner(partner_strip)

            # Setup bus stereo pairs
    ...
    def _rewire_all_controls(self):
        """Rewire all controls after stereo pairs are established"""
        # Rewire channel controls
        for (section, number), strip in self.channel_widgets.items():
            if section == "channel":
                self._wire_strip_controls("channel", number, strip)
    ...
    def on_midi_cc(self, channel_1: int, cc: int, value: int):
        ...
        if typ == "fader":
            ...
            # Update stereo partner if it exists
            if hasattr(strip, 'stereo_partner_strip') and strip.stereo_partner_strip:
                strip.stereo_partner_strip.fader.setValue(value)
        elif typ == "mute" and strip.mute:
            strip.mute.set_from_cc(value)
            # Update stereo partner if it exists
            if hasattr(strip, 'stereo_partner_strip') and strip.stereo_partner_strip and strip.stereo_partner_strip.mute:
                strip.stereo_partner_strip.mute.set_from_cc(value)
            ...
            # Update stereo partner if it exists
            if hasattr(strip, 'stereo_partner_strip') and strip.stereo_partner_strip and strip.stereo_partner_strip.pan:
                strip.stereo_partner_strip.pan.blockSignals(True)
                strip.stereo_partner_strip.pan.setValue(value)
                strip.stereo_partner_strip.pan.blockSignals(False)
                strip.stereo_partner_strip._on_pan_changed(value)        
